...
import { isPromoterInExcel, getPromoterStats, renderStatsCharts } from './stats.js';
...
export class ChatEngine {
    ...
    async _handleEmail(input) {
        ...
        // Verificar si el promotor está en el Excel → mostrar botón Estadísticas
        isPromoterInExcel(collab.nombre).then(esPromotor => {
            if (esPromotor) UI.showEstBtn();
        });
    ...
    openModule(moduleId) {
        // --- Módulo Estadísticas (panel separado) ---
        if (moduleId === 'EST') {
            this._module = 'EST';
            this._state = STATE.IN_MODULE;
            UI.setModBtnActive('EST');
            UI.setBreadcrumb('MOD-EST-007 › Estadísticas');

            const panel = UI.showStatsPanel();
            panel.innerHTML = `<div class="stats-loading">⏳ Cargando datos del promotor...</div>`;

            const nombrePromotor = this._session.collaborator?.nombre || '';

            // Barra de herramientas: Par1, Par6, Par30, Resumen
            const renderToolbar = (tipoActivo) => {
                const toolbar = document.createElement('div');
                toolbar.className = 'stats-toolbar';
                ['PAR1', 'PAR6', 'PAR30', 'ALL'].forEach(tipo => {
                    const btn = document.createElement('button');
                    btn.className = `stats-btn${tipo === tipoActivo ? ' active' : ''}`;
                    btn.textContent = tipo === 'ALL' ? 'Resumen general' : tipo;
                    btn.addEventListener('click', async () => {
                        panel.innerHTML = `<div class="stats-loading">⏳ Calculando ${tipo}...</div>`;
                        try {
                            const stats = await getPromoterStats(nombrePromotor, tipo);
                            panel.innerHTML = '';
                            panel.appendChild(renderToolbar(tipo));
                            const chartsContainer = document.createElement('div');
                            chartsContainer.style.display = 'contents';
                            panel.appendChild(chartsContainer);
                            renderStatsCharts(chartsContainer, stats, nombrePromotor);
                        } catch (e) {
                            panel.innerHTML = `<div class="stats-error">❌ Error al cargar datos: ${e.message}</div>`;
                        }
                    });
            ...
            // Cargar estadísticas iniciales (resumen ALL)
            getPromoterStats(nombrePromotor, 'ALL').then(stats => {
                panel.innerHTML = '';
                panel.appendChild(renderToolbar('ALL'));
                const chartsContainer = document.createElement('div');
                chartsContainer.style.display = 'contents';
                panel.appendChild(chartsContainer);
                renderStatsCharts(chartsContainer, stats, nombrePromotor);
            }).catch(e => {
                panel.innerHTML = `<div class="stats-error">❌ No se pudo cargar las estadísticas: ${e.message}</div>`;
            });
            return;
        }
    ...
    async _handleQuery(input) {
        ...
        if (result) {
            ...
            if (result.botonesTemas) {
                ...
                UI.renderMessage('bot', '_Esta consulta ya fue atendida en esta sesión. Si algo no quedó claro, con gusto profundizo._');
            }
        }
