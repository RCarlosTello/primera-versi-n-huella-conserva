...
export class ChatEngine {
    ...
    _showIASeniorButton(input) {
        ...
        btn.innerHTML = '🤖 Preguntar a Mistral AI';
        btn.title = 'Consulta avanzada usando Mistral AI (requiere conexión y API Key configurada)';
        btn.addEventListener('click', () => {
            btn.remove();
            this._askIASenior(input);
        });
    ...
    async _askIASenior(input) {
        ...
        try {
            ...
            // Para producción, se recomienda Mover esta lógica al backend
            const apiKey = window.MISTRAL_API_KEY || localStorage.getItem('MISTRAL_API_KEY') || 'TU_API_KEY_AQUI';
            ...
            const response = await fetch('https://api.mistral.ai/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        { role: 'system', content: `Eres el Asistente Institucional de Grupo CONSERVA, empresa mexicana de microfinanzas. 
Respondes preguntas sobre: Crédito Solidario (Mujeres de Palabra), Crédito Individual (Tu Negocio con CONSERVA), 
Conserva T Activa, Tu Hogar con CONSERVA, Crédito Paralelo, Caja Chica, Viáticos y Matriz de Sanciones.
El colaborador que pregunta se llama ${collab?.nombre || 'Colaborador'}, puesto: ${collab?.puesto || 'N/A'}. Contexto actual: Módulo ${context.module}, Tema ${context.linea}.
Responde en español, de forma clara, concisa y profesional. Cita la fuente si la conoces.
Si no tienes la información exacta, indícalo claramente y sugiere consultar el manual correspondiente.` },
            ...
            const answer = data.choices?.[0]?.message?.content || 'No pude obtener una respuesta de Mistral AI.';

            renderMessage('bot',
                `🤖 **IA Mistral:** ${answer}\n\n` +
                `_Respuesta generada por inteligencia artificial (Mistral API). Verifica con el manual oficial para decisiones importantes._`,
                'Inteligencia Artificial — Mistral AI'
            );
        } catch (e) {
            console.error('Mistral Error:', e);
            if (e.message === 'falta_api_key') {
                renderMessage('bot', '⚠️ Falta la API Key de Mistral. Por favor configúrala ejecutando en consola: `localStorage.setItem("MISTRAL_API_KEY", "tu-key-aqui")` y refresca el sitio.');
            } else {
                renderMessage('bot', '⚠️ No fue posible conectar con Mistral AI. Verifica tu conexión o API Key e intenta de nuevo.');
            }
        } finally {
            hideTyping();
        }
