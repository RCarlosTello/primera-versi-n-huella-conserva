...
// ── Módulo Estadísticas ────────────────────────────────────
const estBtn = document.getElementById('nav-est');

export function showEstBtn() {
    if (estBtn) { estBtn.classList.remove('hidden'); estBtn.disabled = false; }
}

export function hideEstBtn() {
    if (estBtn) { estBtn.classList.add('hidden'); estBtn.disabled = true; }
}

/** Muestra el panel de estadísticas (ocultando el área de chat). */
...
/** Oculta el panel de estadísticas y muestra el chat normal. */
